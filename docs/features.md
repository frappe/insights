...
| Slug | Feature |
|---|---|
| shared.dashboard-link | A user makes a dashboard Public and a logged-out visitor opens its link and sees its charts. |
| shared.chart-link | A user makes a chart Public and a logged-out visitor opens its link. |
...
| shared.rows-are-the-owners | A visitor on a public chart sees the rows its owner can see, because a Public chart cannot apply each reader's permissions. |
| shared.publish-needs-share | Only a user with share access can widen who may read a chart or dashboard, Public included; narrowing is a plain write. |
| shared.chart-on-public-dashboard | A chart on a Public dashboard opens for a visitor through that dashboard and draws its owner's rows. |
| shared.public-methods-bounded | A visitor reads only through the View endpoints and runs no document method. |
| shared.no-drill | A visitor sees a public chart but cannot drill into it. |
...
| permissions.chart-apply-user-permissions | A chart filters its rows by each reader's permissions while Apply User Permissions is checked, the default, and by its owner's when not. |
...
| permissions.error-is-text | An error message from the server shows as text, never as markup. |
