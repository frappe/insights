...
Accepted, not yet implemented. Amended 2026-08-09: the decision stands, the sequencing does not. This config change follows the render swap instead of leading it, and the swap has shipped ([`charts-render-through-frappe-ui`](charts-render-through-frappe-ui.md)). See *Amendment* at the end.
...
The render swap was ruled blocked on 2026-08-05 by three gaps: combo charts, a numeric x-axis, and Map, Bubble and Sankey, with the raw ECharts escape hatch missing. That described frappe-ui as it stood then. In frappe-ui's charts v2, combo charts are a per-series `type` of `bar`, `line` or `area`. `ScatterChart` covers Bubble, with `sizeColumn` and quadrant dividers as reference lines. `SankeyChart` ships. The escape hatch exists as `echartOptions`, deep-merged at chart, axis and series level. The render swap is not blocked.
...
1. **The ordering is reversed.** The render swap goes first, starting with the
   ...
   per-type adapter functions written against the old slot names and then
   rewritten. It is capped by building the adapter as one module with a function
   per type, so the rewrite changes function bodies and not structure.
...
The seam and the ordering are recorded in [`charts-render-through-frappe-ui`](charts-render-through-frappe-ui.md).
