...
A Number card should be configured, not compiled. This map records how a number card stops being a hand-built query pipeline.
...
- **A window is labeled by the date it starts on.** The dimension could have
  labeled windows `current` and `previous`. Naming them by their start date
...
- **A row in two windows belongs to both.** Spans overlap whenever the span is
  ...
  against last year. Labeling a row by the oldest window holding it made the
...
- **A reading is its own execution.** Modelling each card element as an
  ...
  multi-source readings become unfilterable. The card's drill identity is a
...
- **A new "Look up Value" operation.** A dedicated step for "bring the target
  ...
  says the same thing with no new concept, and generalizes to every cross-grain
