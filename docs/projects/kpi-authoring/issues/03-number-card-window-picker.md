...
The sparkline. A windowed card returns two rows, so a real series needs a second execution. Both questions that were open here are now answered — the card gets its own execution, and it draws the window split one grain finer. Ticket 08 owns it. Leave the existing sparkline behavior as it is.

## Notes

Build on frappe-ui controls and semantic tokens, matching the existing value settings the earlier commits landed (`412fa6de`).

Convention worth defaulting on, from the handoff: a flow measure (revenue, profit) defaults to comparing against the same window last year. A balance (cash, receivables) defaults to the previous window. Defaulting is enough — do not build a measure-type field for it in this ticket.
