...
- **A write's answer confirms the write, and does not report other changes.** `mergeWriteAnswer` takes a field from the answer only where the answer differs from what was sent. Landed in `f39de6c9d`, before this map was written. It is what makes a silent reload safe: a document that reloads itself no longer looks edited.
