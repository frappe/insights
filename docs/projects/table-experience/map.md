...
- `frontend/src2/components/DataTable.vue` is mounted from eight places in three roles: authoring (`QueryBuilderTable`), preview (`ChartBuilderTable`, drill, data source, the CSV upload dialog, native and script editors) and reading (`TableChart` on a card).
...
- The filter row builds `contains` adhoc filters and re-runs, whatever the row count: `QueryDataTable.vue` used to switch on it (`isSinglePage`) and one box meant two things. On a card (`TableChart`) there is still no server path at all.
- The cache line ("from cache" / "fetched in Ns") is written once, in `components/result_pane/ResultStatus.vue` reading `status.ts`. `QueryExecutionStatus.vue` and the copy in `ScriptQueryEditor.vue` are gone — every editor mounts `ResultPane` now.
...
- **The filter picker is settled**: see [one filter picker for every host](../../adr/one-filter-picker-for-every-host.md). Tickets 01 and 02 closed into it. The builder's column filter still wires it.
...
- **Three frappe-ui changes landed on `picker-gaps`:** `DateCalendar` and `DateRangeCalendar` ship from `frappe-ui/experimental`, each owning its selection — which reverses the ask to export `CalendarPanel`, since the panel left every consumer rebuilding the selection around it. `Popover` gains `autoFocus` and `trigger: 'manual'`. `ChartContainer` centers its actions on the title line. The `useInputClasses` ask is won't-fix, and `ContentEditable` carries its one combination by hand. — 07
