...
**The app did not build on the beta.62 pin**: the calendars, `autoFocus` and the composable were all unreleased. `wt link frappe-ui` was what built it. `frontend/package.json` now pins the pushed commit that carries them, which is what CI builds against (`.github/workflows/frontend-build.yml`), so the link is no longer needed — a release is still what the pin should become.

## Follow-ups from the first consumer

The filter picker read the branch through the link. Two things the calendars do not yet say:
