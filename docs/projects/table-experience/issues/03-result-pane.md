...
A result pane that the builder, native, script, drill and data source screens all mount. Three bands:
...
Settled on the prototype `FrameProto.vue`, which lives on branch `try/table-proto` and on no other, variant `page`, 2026-09-09. Copy it, do not reinterpret it.
...
- **Paging is the pane's.** `DataTable`'s footer slot exposes no cursor, so the pane owns `usePagination`, slices the rows, and passes `current-page` down for the row gutter. A no-op `on-page-change` puts the table in server-paged mode, so Next never disables. Do not pass one for client paging.
...
The chart builder mounts the pane under the card, and drops the toolbar it drew above it. The card's own header is the page header: the title frappe-ui's `ChartContainer` already draws, with Refresh and the chart menu in its `actions` slot, passed down from the builder through `ChartRenderer` and `ChartChrome`.
