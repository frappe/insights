...
- **Drop the `v3` suffix** from Query, Chart, Dashboard and Dashboard Chart before ERPNext commits its first file. The doctype name is in each file's path and body, and it lives in another app's repo, so no patch can move it later. The v2 doctypes are already gone on `feat/islands`.
