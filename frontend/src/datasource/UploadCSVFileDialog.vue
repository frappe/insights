...
const show = computed({
	get: () => props.show,
	set: (val) => (!val && reset()) || emit('update:show', val),
})
...
<template>
	<Dialog :options="{ title: 'CSV Import' }" v-model="show">
		<template #body-content>
			...
			<div v-if="columns?.length" class="relative max-h-[15rem] overflow-y-auto text-base">
				<div
					class="sticky right-0 top-0 z-10 flex h-8 w-full cursor-pointer items-center space-x-8 bg-white pr-8 text-xs uppercase text-gray-500"
				>
		...
		<template #actions v-if="table.file">
			<Button
				appearance="primary"
				@click="submit"
