<template>
	<div class="flex h-full min-h-[12rem] w-full flex-1 p-4">
		...
		<div class="flex flex-1" ref="chart_container">
			<div
				v-if="!unsaved_chart?.type"
				class="ml-4 flex flex-1 items-center justify-center rounded-md border-2 border-dashed border-gray-200 text-sm font-light text-gray-400"
			>
				<p>Select Chart Type, Dimension & Measure to display a chart</p>
			</div>
		</div>
	</div>
</template>
