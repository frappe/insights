<template>
	<Popover :show="is_open" placement="right-start">
		<template #target>
			...
			>
				{{ menu_item.label }}
				<FeatherIcon v-if="is_open" name="chevron-right" class="ml-auto -mr-1 h-3 w-3" />
			</span>
		</template>
		<template #content>
			<div class="column-menu-item-popover-content ml-5 w-[28rem] rounded-md bg-white shadow-md ring-1 ring-gray-200">
	...
	methods: {
		apply_condition(conditions) {
			...
			if (conditions && conditions.length) {
				...
				this.column.aggregation_condition = conditions
