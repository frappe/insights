import { formatNumber, getFormatUnits, getShortNumber, guessPrecision } from '../helpers'
import type { NumberFormat } from '../types/chart.types'
import { getRowCurrency } from '../query/helpers'
import type { DataFormat, QueryResultRow } from '../types/query.types'
 ...
 *   2. the Measure's own unit — a currency prints the symbol of the code its
 *      rows carry, or the site's, and a percent is scaled by 100 and prints `%`;
...
export function numberFormatOf(
	...
	const unit = getFormatUnits(measure?.format, carriedCurrency(rows, measure?.measure_name))
...
/** The one resolver. `rows` are the rows the currency code is read off. */
export function numberFormatter(
	config?: NumberFormatConfigSource | null,
	measure?: FormattedMeasure | null,
	rows?: QueryResultRow[],
): NumberFormatter {
	const format = numberFormatOf(config, measure, rows)
 ...
 */
export function carriedCurrency(
	rows?: QueryResultRow[],
	measure_name?: string,
): string | null | undefined {
	if (!rows?.length || !measure_name) return undefined
	const codes = new Set(rows.map((row) => getRowCurrency(row, measure_name)))
...
export function printNumber(value: any, format: ResolvedNumberFormat): string {
	...
	// The magnitude is printed on its own so the sign stands outside the unit:
	// `-$1,234`, not `$-1,234`. A value that rounds away to zero prints no sign,
	// and what "away to zero" looks like is zero under the same settings — the
	// digits are the locale's, so no digit of ours can be tested for.
	const print = (magnitude: number) =>
		format.shorten
			? getShortNumber(magnitude, format.decimals ?? SHORT_DECIMALS)
			: formatNumber(magnitude, format.decimals ?? guessPrecision(Math.abs(scaled)))
	const printed = print(Math.abs(scaled))
	const sign = scaled < 0 && printed !== print(0) ? '-' : ''
	return `${sign}${format.prefix ?? ''}${printed}${format.suffix ?? ''}`
}
