...
//
// The pipeline is the server's — the same cut it ran to answer the level — so
...
const props = defineProps<{
	...
	title: string
	/** what the surface's own filters narrowed to, which the cut does not carry */
...
query.doc.title = __('{0} — Drill Down', props.title)
...
// The reader's own filters over these rows are held here, so they last exactly
// as long as the level does: a new rows level mounts a new component, and
// closing the dialog unmounts this one. Nothing is carried back to the surface.
//
// Every column here is a plain column — these are the cut source rows, not a
// summarize — so a number operator is a `WHERE` like any other.
const filters = ref<Filter[]>([])
const columns = computed(() => query.result.columns || [])

// A filter is left out of its own value list, or picking a second value would be
// impossible: the list is read off rows the reader’s other rules narrowed, and
// the rule on this column has already narrowed them to the one value it holds.
function valuesProvider(column: QueryResultColumn) {
	return (search: string) => {
		const others = filters.value.filter((filter) => filter.column.name !== column.name)
		return query.getDistinctColumnValues(column.name, search, 20, adhocFiltersWith(others))
	}
}

// The numbers a number operator offers to pick from, cut from what this column
// actually holds under the reader's other rules.
function rangeProvider(column: QueryResultColumn) {
	const others = filters.value.filter((filter) => filter.column.name !== column.name)
	return query.getColumnRange(column.name, adhocFiltersWith(others))
}
...
function adhocFiltersWith(own: Filter[]): AdhocFilters {
	const routed = props.adhocFilters || {}
	const routedGroup = routed[query.doc.name]
	const group = toFilterGroup(own)
	if (!own.length) return { ...routed }
	return {
		...routed,
		[query.doc.name]: routedGroup
			? filter_group({
					logical_operator: 'And',
					filters: [...routedGroup.filters, ...group.filters],
			  })
			: group,
	}
}
...
watch(filters, () => {
	query.adhocFilters = adhocFiltersWith(filters.value)
