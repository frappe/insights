...
describe('a query result read as a chart', () => {
	...
	it('reads a summarize as the rows and values a Table declares', () => {
		...
		// a cell click pins what the operation grouped by, as it would on a card
...
describe('what a funnel stage pins', () => {
	...
	it('pins the Label column, when the stages are the rows of a Dimension', () => {
		const funnel = funnelChart({
			dimension: 'stage',
			measure: 'count',
			stages: [{ stage: 'Leads', value: 100 }],
		})
		const chart = { chart_type: 'Funnel', config: funnel.config } as DrillChart

		expect(declaredDimensionColumns(chart)).toEqual(['stage'])
		expect(segmentOf(chart, { column: 'count', row: funnel.result.rows[0] }).filters).toEqual([
			{ column: 'stage', operator: '=', value: 'Leads' },
		])
	})
	...
	it('pins nothing but the stage, when the stages are Measures', () => {
		...
		expect(declaredDimensionColumns(chart)).toEqual([])
		const segment = segmentOf(chart, { column: 'leads', row: funnel.result.rows[0] })
		expect(segment.filters).toEqual([])
		expect(segment.pins).toEqual([])
		expect(segment.measure).toBe('leads')
	})
})
