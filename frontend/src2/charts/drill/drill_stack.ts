...
export type DrillFilter = {
	...
	/** A list where one segment stands for several values of its column. */
...
function declaredDimensions(chart: DrillChart): DeclaredDimensions {
	...
	switch (chart.chart_type) {
		...
		case 'Funnel': {
			...
			return {
				...empty,
				rows: stageMeasures.length ? [] : dims([funnel.label_column]),
				measures: nums(stageMeasures.length ? stageMeasures : [funnel.value_column]),
			}
		}
		case 'Table': {
...
/**
 * A query builder's own result, read as a Chart. The last aggregating operation says
 ...
 * server cuts. The client says what was clicked and nothing about how to
 * match it, which is why it sends literals.
 */
function filterForDimension(dimension: Dimension, value: any): DrillFilter {
	...
	// One segment can stand for several values of its column — a map folds two
	// spellings of a region into one shape — and the drill pins the segment, not
	// one of the rows under it.
...
function labelForDimension(
	...
	if (value === null || value === undefined || value === '') return __('(blank)')
	if (Array.isArray(value)) {
		return value.map((one) => labelForDimension(dimension, one, label)).join(', ')
	}
