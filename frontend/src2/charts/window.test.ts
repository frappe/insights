...
describe('the periods the picker writes', () => {
	...
	// @feature charts.number-period-last-n
	...
	// @feature charts.number-period charts.number-period-include-current
	...
	it('leaves a grain it has no choice for standing as its own choice', () => {
		// An hourly card, authored before periods existed. It echoes as a grain, so
		// picking it back writes the grain it was read from and not a span.
		expect(choiceOfPeriod({ grain: 'hour' })).toBe('grain:hour')
		expect(periodOfChoice('grain:hour')).toEqual({ grain: 'hour' })
		expect(labelOfChoice('grain:hour')).toBe('hour')
	})
})
...
describe('how a window prints', () => {
	...
	it('names the whole run of a span covering several periods', () => {
		// Printing `June, 2026` for a June-to-August span reads as June's own
		// number, which is the reading the span exists to replace.
...
describe('the last-N sentence', () => {
	// One message, split where the count goes, so a translation orders the whole
	// sentence rather than receiving "Last" and a unit as two strings.
	// @feature charts.number-period-last-n
	it('reads as one sentence with the count in it', () => {
		expect(lastSpanSentence('month', 3).join('3')).toBe('Last 3 months')
	})

	// @feature charts.number-period-last-n
	it('says the unit once for a count of one', () => {
		expect(lastSpanSentence('month', 1).join('1')).toBe('Last 1 month')
	})
})
