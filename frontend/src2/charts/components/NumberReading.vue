...
<template>
	...
	>
		...
		>
			<!-- The state every other chart type is given by the chrome around its
			     plot. A card is its own chrome, so it says it here, in the words
			     the chrome says it in. -->
			<template v-if="props.empty && !card.missing" #empty>
