...
// whose values disagree about the period is two cards.
...
const choices = computed(() => {
	...
	return [
		...groups,
		{
			group: labelOfChoice(choice.value),
			hideLabel: true,
			options: [{ label: labelOfChoice(choice.value), value: choice.value }],
		},
	]
})
...
<template>
	<div class="flex flex-col gap-2">
		<InlineFormControlLabel :label="__('Period')">
		...
		<template v-if="props.hasDateColumn && span?.shape === 'last'">
			...
			<InlineFormControlLabel>
				<div class="flex items-center gap-1.5">
					...
					/>
					<span class="text-p-sm text-ink-gray-5">
						{{ lastSpanSentence(span.unit, span.count || 1)[1] }}
					</span>
				</div>
			</InlineFormControlLabel>
