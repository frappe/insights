...
// three readings of one question. `InputGroup` holds them as one control: a
// name under each field, and a placeholder that states what the field inherits.
...
<template>
	...
	<InlineFormControlLabel :label="__('Format')" control-width="10rem">
		<InputGroup :fields="fields" @update="writeField" />
	</InlineFormControlLabel>

	<!-- the inherited value, like the three fields beside it: a card shortening
	     because the chart does must not read as a card that does not -->
	<Toggle
		:label="__('Shorten')"
		:modelValue="shortens"
