...
function readingOf(
	...
	// the newest row is the row behind the card, so its code is the card's
	const format = numberFormatOf(config, measure, rows.slice(-1))
	...
	const negativeIsBetter = options.negative_is_better
 ...
 * the data moved: v2 flips the colors for a Measure where down is better, so
