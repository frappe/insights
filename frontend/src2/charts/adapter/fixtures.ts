...
type MeasureSpec =
	...
	| {
			...
			axis?: 'right'
			/** The unit the Measure states, which decides how its readings print. */
			format?: DataFormat
			color?: string
			dataLabels?: boolean
			area?: boolean
...
function resultWith(columns: QueryResultColumn[], rows: Record<string, any>[]): QueryResult {
	return {
		...
		rows,
		// A parallel array, the way the query store builds it: the drill
...
function toSeries(measure: MeasureSpecObject): Series {
	return {
		...
		...(measure.mark ? { type: measure.mark as Series['type'] } : {}),
...
export type DonutChartSpec = {
	title?: string
	/** The Dimension the segments are named after. */
	category: DimensionSpec
	measure: string
	/** The segments as the server sent them: biggest first. */
...
type FunnelStageSpec = { stage: string; value: number }
...
type SankeyFlowSpec = { source: string; target: string; value: number }
...
type HeatmapCellSpec = { x: string; y: string; value: number }
...
type NumberValueSpec = {
	...
	shorten?: boolean
	color?: string
	/** A Measure where a fall is better, e.g. churn. */
...
export type NumberChartSpec = {
	...
	/** A span the card reads, for a card grouped by span rather than by grain. */
 ...
 * back oldest first, so the one comparison period is the row before the last.
...
type TableMeasureSpec = string | { name: string; format?: DataFormat }
