...
export function adaptBubbleChart(input: ChartAdapterInput): ChartFiller | undefined {
	...
	const props: ScatterChartProps = {
		...
		yAxis: { format: numberFormatter(config, config.yAxis, input.result.rows) },
		format: numberFormatter(config, config.size_column, input.result.rows),
	}
	...
	const label = config.dimension?.dimension_name
