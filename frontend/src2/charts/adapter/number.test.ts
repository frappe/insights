...
describe('a Number Chart with several values', () => {
	...
	it('stands the grid up from the config alone, so the cards wear the states', () => {
		...
		// Nothing to drill into: no row stands behind the reading. The card reads
		// as unclickable on the same answer, so a pointer over it promises nothing.
		expect(filler?.drillDown).toBeUndefined()
		expect(filler?.props.drillable).toBe(false)
	})
})
...
describe('a Chart naming one Measure twice', () => {
	...
	it('draws it once, because a reading is named by its Measure', () => {
		...
		expect(cards.map((card: any) => card.column)).toEqual(['Revenue'])
	})
})
