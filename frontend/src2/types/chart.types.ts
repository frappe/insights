...
export type NumberComparison = {
	 ...
	 * both of which the card's own period is what fetches — a span shifts
	 * itself back, a grain already holds the row. `constant` is a fixed number,
...
export type NumberColumnOptions = {
	...
	suffix?: string
	color?: string
	/** A fall is better, e.g. churn or cost. Flips the comparison's colors. */
	negative_is_better?: boolean
	/** What the reading is aimed at. A target belongs to the reading, not to the chart. */
	target?: NumberTarget
...
export type HeatmapChartConfig = NumberFormatConfig & {
	...
	/** The ends of the color scale. Left out, the data's own ends are used. */
	min?: number
	max?: number
}
