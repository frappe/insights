 ...
 * only choice it has is which end is heavy. The stored name says that
...
/**
 * The two names this option had when the scale ran red to green. Each named
 ...
 * the bottom of a magnitude scale is an absence, so a zero shows
 * nothing rather than the heaviest thing on the page.
 *
...
/**
 * The deep end of a scale, as a percentile `fillAt` reads.
 *
 * A column of one value has no range to rank against, and the reader still
 * asked for a scale over it, so it reads as the heavy end — whichever end the
 * direction puts that at.
 */
export function deepEnd(direction: ColorScaleDirection = 'ascending') {
	return direction === 'descending' ? 0 : 100
}
...
export function fillAt(scale: (CellFill | undefined)[], percentile: number): CellFill | undefined {
	if (!scale.length || !Number.isFinite(percentile)) return undefined
