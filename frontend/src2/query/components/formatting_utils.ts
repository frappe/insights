...
export function rulesByColumn(
	...
	formats.forEach((format) => {
		...
		columnNames
			.filter(
				(name) =>
					name === target || measureOf(name) === target || name.endsWith(`___${target}`),
			)
			.forEach((name) => {
...
export function applyRule(value: any, rule: cell_rules): boolean {
...
export function applyTextRule(value: any, rule: text_rules): boolean {
...
export function applyDateRule(value: any, rule: date_rules): boolean {
	...
	switch (rule.operator) {
		...
		case 'is_last_month':
			...
			return (
				dateVal.getMonth() === lastMonth.getMonth() &&
				dateVal.getFullYear() === lastMonth.getFullYear()
			)
		case 'is_next_month':
			const nextMonth = new Date(today)
			nextMonth.setMonth(today.getMonth() + 1)
			return (
				dateVal.getMonth() === nextMonth.getMonth() &&
				dateVal.getFullYear() === nextMonth.getFullYear()
			)
		case 'is_this_year':
