...
<template>
	<Dialog :modelValue="show" @update:modelValue="show = $event" :options="{ title: 'Variables', size: 'lg' }">
		<template #body>
			<div class="bg-white px-4 pb-6 pt-5 sm:px-6">
				...
				<div class="flex flex-col overflow-hidden">
					<div class="relative flex max-h-[20rem] flex-col overflow-y-auto">
						<div class="sticky top-0 flex gap-x-2 border-b bg-white py-2 text-sm font-medium text-gray-600">
						...
						<div v-for="(variable, index) in variables" :key="index" class="flex gap-x-2 border-b border-gray-100 py-1">
							...
							<div class="flex w-10 justify-end">
								<Button variant="ghost" icon="x" size="sm" @click="removeVariable(index)" />
							</div>
						</div>
						
						<div v-if="variables.length === 0" class="flex justify-center py-8">
							<span class="text-sm text-gray-400">No variables added</span>
						</div>
					</div>
