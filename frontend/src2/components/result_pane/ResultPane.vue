...
const pagination = usePagination({
	pageSize,
	rowCount: computed(() => loadedRows.value.length),
...
const pageRows = computed(() => matchedRows.value.slice(0, pagination.endIndex.value))
...
const exportDefaultName = computed(() => {
	const now = new Date()
	return `export_${now.getDate()}_${now.getMonth() + 1}_${now.getFullYear()}`
})
...
<template>
	<div class="flex h-full min-h-0 w-full flex-1 flex-col rounded-4 border border-outline-gray-2">
		<!-- The find is the pane's, since it knows the rows and the columns, but
		     a host with a header of its own places it there. Outside the clip
		...
		>
			<!-- one slot, three states: the body is replaced, never pushed down by a banner -->
			<div
				v-if="bodyState"
