 ...
 *
 * A stored layout is not always compact. Compaction is a preference the surface
 ...
 *
 * `label` is read when it is asked for: translations land after this file is
 * imported, so a label built here at module scope would keep its English.
 */
...
export function placementsFor(
	...
	if (!breakpoint || breakpoint === BASE_BREAKPOINT) {
		// Settled here too, not only at a narrower breakpoint: several charts added
		// at once are dropped on one box, and nothing downstream pushes a cell down.
	...
	const merged = applyCellRules(
		items.map((item) => {
			...
			if (!stored) return byId.get(item.layout.i)!
...
/**
 * Where a breakpoint would put one item if it stored nothing for it.
 *
 * The item is placed among the others as they stand, because a derived cell is
 ...
 * puts it.
 */
export function derivedPlacement(
	items: WorkbookDashboardItemLayout[],
	key: BreakpointKey,
	index: number,
	rules?: CellRules,
	options?: { verticalCompact?: boolean },
): Layout | undefined {
	...
	let bare = items
	if (item.layouts?.[key]) {
		const { [key]: stored, ...rest } = item.layouts
		bare = [...items]
		bare[index] = { ...item, layouts: rest }
	}

	const placed = placementsFor(bare, key, rules)
 ...
 *
 * A narrower breakpoint's entry is dropped when the cell lands where `derived`
 * (where the breakpoint puts it with nothing stored) already puts it. Storing
 * it there would pin it: the card would keep that box while the wide layout
 * moved on, and nothing else could ever unpin it.
 */
export function writePlacement(
	...
	if (derived && sameBox(derived, layout)) {
		if (!item.layouts?.[key]) return
		const { [key]: dropped, ...rest } = item.layouts
		item.layouts = rest
		return
	}
