...
function slotStyle(i: string) {
	...
	return {
		...
		// a cell no wider than nothing has no own width to shift by, and the
		// division would be infinite
		across: `${cell.w ? (cell.x / cell.w) * 100 : 0}%`,
...
<template>
	<!-- `isolate` so the held cell's stacking is settled among its siblings and
	     reaches nothing outside this grid -->
	<div ref="container" class="relative isolate w-full" :style="{ height: `${height}px` }">
		...
		<!-- A cell is a box this grid positions, with no role and no name of its
		     own, so the end-to-end suite has nothing else to find one by. -->
		<div
			v-for="(layout, index) in layouts"
			:key="layout.i"
			:style="cellStyle(layout)"
			data-testid="dashboard-cell"
