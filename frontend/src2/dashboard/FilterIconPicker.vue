...
const groups = computed<FilterIconGroup[]>(() => {
	...
	if (!term) return filterIconGroups()
	return filterIconGroups()
		.map((group) => ({
			label: group.label,
			...
			<div class="flex w-64 flex-col p-2">
				...
				<div class="mt-2 max-h-64 overflow-y-auto">
					<div v-for="group in groups" :key="group.label" class="mb-2 last:mb-0">
						<p class="px-1 pb-1 text-xs text-ink-gray-5">{{ group.label }}</p>
						<div class="flex flex-wrap">
							<button
								...
								class="flex h-8 w-8 items-center justify-center rounded-4 hover:bg-surface-gray-3 focus:outline-none focus-visible:ring-2 focus-visible:ring-outline-gray-3"
				...
				<button
					v-if="selected"
					type="button"
					class="mt-1 rounded-4 border-t px-2 py-1 text-left text-base text-ink-gray-6 hover:bg-surface-gray-3 focus:outline-none focus-visible:ring-2 focus-visible:ring-outline-gray-3"
					@click="clear(close)"
				>
					{{ __('Remove icon') }}
				</button>
