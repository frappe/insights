...
def _doctype_named_by(origin: Origin, pins: Pins) -> str | None:
    ...
    if df and df.fieldtype == "Dynamic Link" and df.options:
        ...
        return pinned if pinned and frappe.db.exists("DocType", pinned, cache=True) else None
...
def _trace(operations: list[dict], seen: set) -> tuple[str | None, dict[str, Origin], Pins] | None:
    ...
    said otherwise. The default falls away at the first summarize: after it, only
    ...
    for operation in operations:
        # `custom_operation` returns whatever its expression builds, which can be
        # a whole other table, so nothing after it can be followed either
        if operation.get("type") in ("union", "sql", "code", "custom_operation"):
            return None
...
def _follow(
    ...
    """What each operation does to where the columns come from."""
    ...
    for operation in operations:
        ...
        if type == "rename":
        ...
        elif type == "sql_column":
            # raw SQL makes a value that belongs to no table. Recorded rather
            # than left out, or it would fall through to the base table under a
            # name that table has a field of. The name is not sanitized, the
            # same way the operation writes it.
            origins[operation.get("new_name")] = None
            ...
            taken = set(origins) | _columns_of(default)
            for column in operation.get("select_columns") or []:
                name = column.get("column_name")
                # a name the left side already answers is still the left side's:
                # ibis renamed the right table's copy of it
                if name in taken:
                    continue
            ...
            # narrowed rather than the column name it used to say it. An `Or`
            # group narrows to none of its rules on its own, so it pins nothing
            if type == "filter_group" and operation.get("logical_operator") != "And":
                continue
...
def _columns_of(table_name: str | None) -> set[str]:
    """The column names a site table answers, so a join's duplicates are spotted.

    ibis renames the right table's copy of a name the left side already holds,
    and the trace has no result to read the names off.
    """
    if not table_name or not table_name.startswith("tab"):
        return set()
    ...
    return {
        "name",
        *(field["fieldname"] for field in std_fields),
