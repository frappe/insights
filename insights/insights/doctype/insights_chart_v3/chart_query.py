...
from insights.insights.query_builders.sql_functions import span_unit
...
def derive_operations(chart_type: str, query: str, config: dict | None) -> list[dict]:
    ...
    _add_period_order(operations, chart_type, config)
    return operations
...
def sparkline_grain(chart_type: str, config: dict | None) -> str | None:
    ...
    span needs it: a grain card already returns one row per period, so its own
    readings are the series.
    """
    ...
    return SPARKLINE_GRAINS.get(_span_unit(_period(config).get("span")))
...
def sparkline_operations(chart_type: str, query: str, config: dict | None) -> list[dict]:
    ...
    """
    grain = sparkline_grain(chart_type, config)
    if not grain:
        return []
...
def column_granularity(operations: list[dict]) -> dict:
    ...
    The grain lives in the summarize or pivot operation. A viewer never receives
    the operation itself, so this map is what it gets instead, one grain per column.
    """
    granularity = {}
    for operation in operations:
        ...
        for dimension in dimensions:
            if dimension.get("granularity"):
                granularity[result_column(dimension)] = dimension["granularity"]
...
def config_errors(chart_type: str, query: str, config: dict | None) -> list[str]:
    ...
    malformed = _malformed_slots(config, chart_type)
    if malformed:
    ...
    if chart_type == "Sankey":
        source_column = config.get("source_column") or {}
        target_column = config.get("target_column") or {}
        if not source_column.get("column_name"):
            errors.append(_("Source column is required"))
        if not target_column.get("column_name"):
            errors.append(_("Target column is required"))
        if source_column.get("column_name") and source_column.get("column_name") == target_column.get(
            "column_name"
        ):
            errors.append(_("Source and target cannot be the same"))
        if not (config.get("value_column") or {}).get("measure_name"):
            errors.append(_("Value column is required"))
...
def _period(config: dict) -> dict:
    ...
    A period names one grouping: a `span` filters to a stretch of the calendar
...
def _add_number_operation(operations: list[dict], config: dict):
    ...
    # No period, so no grouping. The date column is the sparkline's axis here, not
    # something the reading is cut by.
    operations.append(_summarize(measures=_number_measures(config), dimensions=[]))


def _add_period_order(operations: list[dict], chart_type: str, config: dict):
    """A card reads the periods its own sort puts within reach, whatever else it is sorted by.

    Both shapes read one row: a grain card the newest period, a span card the
    newest span. A grain card is fetched newest first, because a grain filters
    nothing: a daily card over a year has more periods than a page holds, and
    the two the card reads have to be on the first one. The page is turned back
    over before anybody reads it (see `reads_newest_first`). A span filters, so
    its spans are the only rows there are and they come back oldest first,
    which is where every reader looks for the reading.

    Either way the sort is the card's and not the author's to move: it is
    applied after the config's own sorts and it replaces any the author put on
    the same column. It goes last because the engine ranks by the last sort
    first, and an author sorting the card by a measure must not decide which
    period it reads.
    """
    column_name, direction = _period_order(chart_type, config)
    _add_order_by(operations, column_name, direction)


def _period_order(chart_type: str, config: dict) -> tuple[str, str]:
    ...
    if period.get("span"):
        return result_column(date_column), "asc"
    if period.get("grain"):
        return result_column(date_column), "desc"

    return "", ""


def reads_newest_first(chart_type: str, config: dict | None) -> bool:
    """Whether this card's rows come back newest first.

    Only the grain card. It filters nothing, so its periods can run past a page,
    and the page it is read off has to be the newest one. Every reader takes the
    reading off the last row, so those rows are reversed once they are fetched.

    Every grain card, because the sort that puts them that way is the card's
    own: `_add_period_order` runs after the author's sorts and outranks them.
    """
    if chart_type != "Number":
        return False

    return _period_order(chart_type, _config_for_derivation(config, chart_type))[1] == "desc"


def _add_window_operations(operations: list[dict], config: dict, window: dict, date_column: dict):
    """One row per span, ordered by `_add_period_order` once the author's own sorts are in.
    ...
    two readings comparing against different spans each read their own.

    The dimension is the span itself, not the unit it names. A span of several
...
def _span_periods(span: str) -> int:
    ...
    A forward run counts the same. "next 3 months" covers three, so the period
    before it is three back. One back is a span that overlaps the card's own.
    """
    text = span.lower().strip()
    include_current = "(include current)" in text
    text = text.replace("(include current)", "").strip()
    if not (text.startswith("last ") or text.startswith("next ")):
        return 1
...
def drawn_measures(chart_type: str, config: dict | None) -> list[dict]:
    ...
    return _named_measures(_config_for_derivation(config, chart_type).get("number_columns"))
...
def _comparison_shifts(config: dict, span: str) -> list[dict]:
    """The spans the comparisons ask for, beside the configured one.
    ...
    comparison now means, with nothing stored to go stale.

    Two values comparing against the same span ask for one span, so the same
...
def _comparison_shift(comparison: dict, span: str) -> dict | None:
    """The span one comparison asks for, or nothing when it asks for no
    second span."""
...
def _timespan(window: dict, shift: dict | None) -> dict:
    """One span, as the engine reads it: the span, what it is anchored to, and
...
def config_filter_group(config: dict | None) -> dict | None:
    """The filter operation a chart's config puts on its source query, or nothing.

    Derivation appends this, and so does every surface that answers a question
    about what one of the chart's columns holds: a reader asking a card's filter
    for its values is asking about the card, which reaches no further than what
    the chart published.
    """
    filters = (config or {}).get("filters") or {}
    if not filters.get("filters"):
        return None
    return {"type": "filter_group", **filters}


def _add_filters(operations: list[dict], config: dict):
    filter_group = config_filter_group(config)
    if filter_group:
        operations.append(filter_group)
...
def _add_axis_time_order(operations: list[dict], chart_type: str, config: dict):
    ...
    Before the author's own sorts, because ibis reads the newest sort as the
    primary key. An author who sorted the chart said how they want to read it
    (a top-N by the measure is a ranking, not a timeline), so their sort is the
    primary one and this survives as the tiebreak. A chart nobody sorted runs
    forwards, which is the whole of what this is for.

    A sort the author put on the x column replaces this one rather than joining
    it, and stands where they wrote it: `_add_order_by` keeps one sort per
    column, the newest.
    """
    ...
    column_name = result_column(dimension)
    if not column_name:
        return

    _add_order_by(operations, column_name, "asc")
...
def _add_order_by(operations: list[dict], column_name: str, direction: str):
    ...
    Two order-by operations on one column conflict, so a column already sorted
    keeps one. It is the newest one: the old operation is dropped and the sort
    is appended where the caller asked for it. Donut, Funnel, Heatmap and a
    date axis all sort before the config's own sorts are read, and writing an
    author's sort back into the slot one of those took would pin it ahead of
    every other sort they wrote. ibis reads the newest sort as the primary key,
    so the rest of their list would outrank it.
    """
    if not column_name:
        return
    ...
    operations[:] = [
        operation
        for operation in operations
        if not (operation["type"] == "order_by" and operation["column"]["column_name"] == column_name)
    ]
...
def normalize_number_shapes(config: dict) -> bool:
    ...
    if not isinstance(columns, list):
        # a config this cannot read is one `_slot_errors` reports on, so it has to
        # reach that far rather than raising on the way
        columns = []
    ...
    for index in range(len(columns)):
        ...
        moved = _normalize_comparison(beside) or moved

    if len(options) > len(columns):
        # an option outlives the reading it sits beside, and derivation reads the
        # options: a removed reading would keep adding a measure and a span
        del options[len(columns) :]
        moved = True
...
def _raise_period(config: dict) -> bool:
    ...
    if window and not isinstance(window, dict):
        # a period shape no repair understands is left as it is, so
        # `config_errors` names it instead of this quietly writing over it
        return False
    window = window or {}
...
def _measured_against(options: dict, flag) -> dict:
    ...
    if isinstance(references, list):
        return _from_references(references)

    return {"comparison": {"source": "previous"}} if flag else {}


def _from_references(references: list) -> dict:
...
def _malformed_slots(config: dict | None, chart_type: str | None = None) -> list[str]:
...
# the chart type reads and the config leaves empty. A slot holding the wrong kind
# is the same slot, so a reader is told about it in the same words. Untranslated
# here: the message is built per request, where the reader's language is known.
SLOT_WORDS = {
    "x_axis": "X-axis",
    "split_by": "Split by",
    "y_axis": "Y-axis",
    ...
    "filters": "Filters",
    "number_columns": "Number column",
    "number_column_options": "Reading options",
    "window": "Period",
    "measures": "Measures",
    "rows": "Rows",
    "columns": "Columns",
...
def _malformed_slot_error(slot: str) -> str:
    ...
    """
    word = SLOT_WORDS.get(slot.split(".")[0])
    return _("{0} is malformed").format(_(word) if word else slot)
...
def _slot_errors(value, shape, slot: str) -> list[str]:
    ...
    if isinstance(shape, list):
        if not isinstance(value, list):
            return [_malformed_slot_error(slot)]
...
def result_column(dimension: dict) -> str:
