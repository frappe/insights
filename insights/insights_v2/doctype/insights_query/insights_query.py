...
class InsightsQuery(InsightsLegacyQueryClient, InsightsQueryClient, Document):
    ...
    def results_row_count(self):
        return InsightsQueryResult.get_value(filters={"query": self.name}, fieldname="results_row_count")
    ...
    def update_linked_docs(self):
        ...
        _ = frappe.qb.update(table).set(table.label, self.title).where(table.table == self.name).run()
