    ...
    def process_columns(self):
        ...
        for row in self.query.columns:
            if not row.is_expression:
                ...
                _column = self.column_formatter.format(parse_json(row.format_option), row.type, _column)
            ...
            if row.order_by:
                self._order_by_columns.append(_column.asc() if row.order_by == "asc" else _column.desc())
