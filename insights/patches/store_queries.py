...
def execute():
    ...
    for query in queries_on_query_store:
        tables = frappe.get_all("Insights Query Table", filters={"parent": query}, pluck="table")
        for table in tables:
            if frappe.db.exists("Insights Query", table):
                queries_to_stored.append(table)

    if queries_to_stored:
        (frappe.qb.update(Query).set(Query.is_stored, 1).where(Query.name.isin(queries_to_stored)).run())
