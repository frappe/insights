...
class TestNumberCellExpansion(InsightsIntegrationTestCase):
    ...
    def test_one_unreadable_item_does_not_stop_the_split(self):
        items = [
            "not an item",
            {"type": "chart", "chart": "c", "layout": {"i": "a", "x": 0, "y": 0, "w": 20, "h": 8}},
        ]
    ...
    def test_a_minted_id_does_not_take_one_the_grid_already_holds(self):
        ...
        items = [
            {"type": "chart", "chart": "c", "layout": {"i": "a", "x": 0, "y": 0, "w": 20, "h": 8}},
            {"type": "text", "text": "hello", "layout": {"i": "a-2", "x": 0, "y": 8, "w": 4, "h": 2}},
        ]

        expand_items(items, self.configs(2))

        ids = [item["layout"]["i"] for item in items]
        self.assertEqual(len(set(ids)), len(ids))
        self.assertEqual(ids[:2], ["a", "a-3"])
