"""A span number card's sparkline, from the config to the rows.
...
# the day the card is read on, so the span it covers never moves
ANCHOR = "2026-08-10"
IN_WINDOW = f"{TODO_PREFIX} third"

# two on one day and one on another, plus a row on either side of the span
TODOS = {
...
class TestNumberCardSparkline(InsightsIntegrationTestCase):
    ...
    def test_the_series_splits_the_cards_own_window_by_day(self):
        ...
        # one row per span is what the number is read off
