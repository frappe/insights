...
def value_column(aggregation, measure_name, data_type="Integer"):
    return {
        "column_name": "value",
        "data_type": data_type,
...
def sankey_config(aggregation, measure_name, data_type="Integer"):
    return {
        ...
        "value_column": value_column(aggregation, measure_name, data_type),
    }
...
class TestSankeyAggregationRepair(InsightsIntegrationTestCase):
    ...
    def test_a_count_over_a_column_that_cannot_be_summed_is_left_alone(self):
        ...
        """
        for data_type in ("String", "Date"):
            with self.subTest(data_type=data_type):
                config = sankey_config("count", "count_of_value", data_type)
    ...
    def test_a_count_over_a_column_of_no_stated_type_is_left_alone(self):
        ...
        config = sankey_config("count", "count_of_value")
        config["value_column"].pop("data_type")
        chart = self.create_chart("Sankey", config)
