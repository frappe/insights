...
the rows a saved chart with that shape would give. A saved chart's own
...
The caller already holds the seat that lets them build any query they like, so
a pipeline they send is a pipeline they could have run anyway; the engine
applies their permissions to it either way.
"""
...
def get_chart_data(
    ...
    return {
        ...
        # which columns name a desk record, so the builder's card draws the links
        # a reader will see
